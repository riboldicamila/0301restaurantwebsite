  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 md:py-16">
      <div className="text-center mb-12">
        <h1 className="text-4xl md:text-5xl font-bold text-red-500 mb-4">MAKE A RESERVATION</h1>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 md:p-8 max-w-3xl mx-auto">
        <div className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                <label htmlFor="date" className="block text-gray-700 font-medium mb-1">Reservation Date</label>
                <div className="relative">
                  <input
                    type="text"
                    id="date"
                  ...
                  <div className="absolute z-10 mt-1 bg-white shadow-lg rounded-md border border-gray-200 p-4">
                    <div className="grid grid-cols-7 gap-1">
                      ...
                      {Array.from({ length: 35 }, (_, i) => {
                        const today = new Date();
                        const day = new Date(today.getFullYear(), today.getMonth(), today.getDate() + i - today.getDay());
                        const isAvailable = availableDates.some(d => d.toDateString() === day.toDateString());
                        ...
                        return (
                          <div
                            key={i}
                            onClick={() => isAvailable && !isPast && !isFuture && handleDateSelect(day)}
                            className={`
                              text-center py-1 rounded-md text-sm
                              ${isSelected ? 'bg-red-500 text-white' : ''}
                              ${isAvailable && !isPast && !isFuture ? 'hover:bg-red-100 cursor-pointer' : ''}
                              ${!isAvailable || isPast || isFuture ? 'text-gray-300 cursor-not-allowed' : ''}
                            `}
                          >
                            {day.getDate()}
                          </div>
          ...
          <div>
            <label htmlFor="notes" className="block text-gray-700 font-medium mb-1">Special Requests / Notes (Optional)</label>
            <textarea
              id="notes"
              name="notes"
        ...
        {showError && (
          <div className="mt-6 p-4 bg-red-50 border border-red-500 text-red-700 rounded-md">
            There was an error processing your reservation. Please ensure all required fields are filled.
          </div>
        )}
      </div>
    </div>
